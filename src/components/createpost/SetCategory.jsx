...
function SetCategory({setStep,setPost}) {
  ...
  return (
    ...
    <div className='w-[300px] md:w-[500px] container mx-auto my-5'>
        <h2 className='md:text-xl text-base font-light' >ثبت اگهی</h2>
        <p className='md:text-sm text-xs text-neutral-400 mt-2 md:mt-3'>انتخاب دسته بندی</p>
        <input  onChange={searchHandler}  className='input rounded-none w-full h-6 md:h-9 mt-3 mb-4 md:mb-10 md:mt-6 placeholder:text-sm md:placeholder:text-base  placeholder:text-neutral-400 placeholder:font-extralight ' type="text" placeholder='جست و جو در دسته ها' />
         {Loading ?<Loader/>:
        <>
          {!!category.length ?
          <ul>
           ...
           {categories?.data.map((category)=>(
             <li onClick={()=>{setStep(2), setPost({category:category._id})}} key={category._id} className='cursor-pointer flex flex-row justify-between items-baseline border-b-2 border-solid border-neutral-200 mb-4 pt-2 pb-4 px-2 ' >
              <p className='flex flex-row-reverse text-sm md:text-base'>{category.name}<img className='md:w-6 w-4 ' src={`${category.icon}.svg`} alt="" /></p> 
             <ArrowBackIosNewIcon style={{fontSize:"20px"}} className=' text-neutral-500' />
             </li>
